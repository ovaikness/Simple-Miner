...
{
	FMOD_VECTOR lPosition = {-listenerPosition.y, listenerPosition.z, -listenerPosition.x};
	FMOD_VECTOR lForward  = {-listenerForward.y,  listenerForward.z,   -listenerForward.x};
	FMOD_VECTOR lUp       = {-listenerUp.y,       listenerUp.z,   -listenerUp.x};
	FMOD_VECTOR lVel	  = { 0.f,0.f,0.f };
...
SoundPlaybackID AudioSystem::StartSoundAt(SoundID soundID, Vec3 const& soundPosition, bool isLooped /*= false*/, float volume /*= 1.f*/, float balance /*= 1.f*/, float speed /*= 1.f*/, bool isPaused /*= false*/ , float pitch /*= 1.f*/)
{
	FMOD_VECTOR sPosition = { -soundPosition.y, soundPosition.z, -soundPosition.x };
	FMOD_VECTOR sVel = { 0.f , 0.f , 0.f };
	...
	if (channelAssignedToSound)
	{
		...
		channelAssignedToSound->setPitch(pitch);
		channelAssignedToSound->set3DAttributes(&sPosition, &sVel);
	}
...
bool AudioSystem::IsPlaying(SoundPlaybackID soundPlaybackID)
{
	...
	FMOD::Channel* channelAssignedToSound = (FMOD::Channel*)soundPlaybackID;
	bool isPlaying;
	if (channelAssignedToSound->isPlaying(&isPlaying) == FMOD_RESULT::FMOD_OK)
	{
		return isPlaying;
	}
...
//-----------------------------------------------------------------------------------------------
SoundID AudioSystem::CreateOrGetSound( const std::string& soundFilePath , FMOD_MODE mode /*= FMOD_DEFAULT*/)
{
...
//-----------------------------------------------------------------------------------------------
SoundPlaybackID AudioSystem::StartSound( SoundID soundID, bool isLooped, float volume, float pitch, float balance, float speed, bool isPaused )
{
	if (volume == 0.f) return MISSING_SOUND_ID;
	size_t numSounds = m_registeredSounds.size();
	if( soundID < 0 || soundID >= numSounds )
		return MISSING_SOUND_ID;
...
//-----------------------------------------------------------------------------------------------
// Pitch is frequency multiplier (1.0 == normal)
//	
//	
//
void AudioSystem::SetSoundPlaybackPitch(SoundPlaybackID soundPlaybackID, float pitch)
{
	if (soundPlaybackID == MISSING_SOUND_ID)
	{
		ERROR_RECOVERABLE("WARNING: attempt to set speed on missing sound playback ID!");
